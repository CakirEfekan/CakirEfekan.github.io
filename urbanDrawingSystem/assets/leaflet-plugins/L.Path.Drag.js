...
L.Handler.PathDrag = L.Handler.extend({
  ...
  _onDragStart: function (t) {
    ...
    L.DomEvent.on(document, MOVE[a], this._onDrag, this).on(
      document,
      END[a],
      this._onDragEnd,
      this
  ...
  _onDrag: function (t) {
    ...
    var e = i.x
    var r = i.y
    var s = e - this._startPoint.x
    var o = r - this._startPoint.y
    if (s || o) {
  ...
  _onDragEnd: function (t) {
    ...
    if (i) {
      this._path.fire('dragend', {
        distance: distance(this._dragStartPoint, a)
      })
    ...
    if (this._mapDraggingWasEnabled) {
      if (i) L.DomEvent.fakeStop({ type: 'click' })
  ...
  _transformPoints: function (t, a) {
    ...
    if (i._point) {
      ...
      for (n = 0, e = l.length; n < e; n++) {
        a[n] = a[n] || []
        for (var u = 0, c = l[n].length; u < c; u++) {
          r = f[n][u]
    ...
    : {
        ...
        transformPath: function (t, a) {
          ...
          var n =
            ...
            ' 0 0'
          var e =
            Math.floor(a[4]).toFixed() + ', ' + Math.floor(a[5]).toFixed() + ''
          ...
          if (isNaN(h) || !h) h = 1
          if (isNaN(_) || !_) _ = 1
          var g = (-s / h - 0.5).toFixed(8) + ' ' + (-o / _ - 0.5).toFixed(8)
          i.on = 'f'
          i.matrix = n
          i.origin = g
          i.offset = e
...
L.Canvas.include({
  ...
  transformPath: function (t, a) {
    ...
    if (!i) {
      ...
      this._removePath(t)
      this._redraw()
      e.translate(r * s.min.x, r * s.min.y)
      e.drawImage(this._container, 0, 0)
      this._initPath(t)
      t._containsPoint_ = t._containsPoint
      t._containsPoint = TRUE_FN
    }
    n.save()
    n.clearRect(h.x, h.y, o.x * r, o.y * r)
